...
class Pilot(DynamicArrays):
    ...
    def FMSOrAsas(self):
        ...
        if bs.traf.wind.winddim > 0:
            vwn, vwe = bs.traf.wind.getdata(
                bs.traf.lat, bs.traf.lon, bs.traf.alt)
            asastasnorth = bs.traf.asas.spd * \
                np.cos(np.radians(bs.traf.asas.trk)) - vwn
            asastaseast = bs.traf.asas.spd * \
                np.sin(np.radians(bs.traf.asas.trk)) - vwe
            asastas = np.sqrt(asastasnorth ** 2 + asastaseast ** 2)
        # no wind, then ground speed = TAS
        else:
            asastas = bs.traf.asas.spd

        # Determine desired states from ASAS or AP. Select asas if there is a
        # conflict AND resolution is on.
        self.trk = np.where(
            bs.traf.asas.active,
            bs.traf.asas.trk,
            bs.traf.ap.trk)
        self.spd = np.where(bs.traf.asas.active, asastas, bs.traf.ap.tas)
        self.alt = np.where(bs.traf.asas.active, bs.traf.asas.alt,
                            bs.traf.ap.alt)
        self.vs = np.where(bs.traf.asas.active, bs.traf.asas.vs, bs.traf.ap.vs)

        # ASAS can give positive and negative VS, but the sign of VS is
        # determined using delalt in Traf.ComputeAirSpeed
        # Therefore, ensure that pilot.vs is always positive to prevent
        # opposite signs of delalt and VS in Traf.ComputeAirSpeed
        ...
        if bs.traf.wind.winddim > 0:
            ...
            angle = Vw * np.sin(drift) / np.maximum(0.001, bs.traf.tas)
            steer = np.arcsin(np.minimum(1.0, np.maximum(-1.0, angle)))
            # desired heading
    ...
    def FlightEnvelope(self):
        ...
        self.spd = np.where(bs.traf.limspd_flag, vcas2tas(
            bs.traf.limspd, bs.traf.alt), self.spd)
        ...
        # below crossover altitude: CAS=const, above crossover altitude:
        # MA = const
